...
def before_scenario(context, scenario):
    ...
    context.driver.implicitly_wait(implicit_wait)
    context.driver.get(base_url)
    logger.info("Browser Opened & Maximized Successfully")

    # ✅ Attach page object AFTER driver is created
    context.signup_page = SignupPage(context.driver)
...
def after_scenario(context, scenario):
    ...
    if scenario.status == "failed":
        ...
        screenshot_path = ScreenshotUtil.capture_screenshot(context.driver, scenario.name)
