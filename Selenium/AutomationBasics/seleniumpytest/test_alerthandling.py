...
def test_simple_js_confirmok(driver):
    driver.find_element(By.XPATH,"//button[text()='Click for JS Confirm']").click()
    alert=driver.switch_to.alert
    time.sleep(3)
    assert alert.text=="I am a JS Confirm","Alert text was wrong"
...
def test_js_prompt(driver):
    driver.find_element(By.XPATH, "//button[text()='Click for JS Prompt']").click()  # FIXED: correct button
    alert = driver.switch_to.alert
    assert alert.text == "I am a JS prompt", "Alert text was wrong"
    time.sleep(2)
    alert.send_keys("Python Selenium")  # works only for prompt
    alert.accept()
    result = driver.find_element(By.ID, "result").text
