...
class InventoryPage(BasePage):
    logger.info(f"Getting inventory page")
    ...
    # ===== Page actions =====
    def is_loaded(self):
        logger.info(f"The page is loaded")
        return self.is_visible(self.INVENTORY_CONTAINER)

    def get_page_title(self):
        logger.info(f"The page title is loaded")
        return self.get_text(self.PAGE_TITLE)

    def get_products_count(self):
        logger.info(f"getting the products")
        return len(self.driver.find_elements(*self.INVENTORY_ITEMS))

    def add_first_product_to_cart(self):
        logger.info(f"Adding the the first product")
